 ...
 * @LastEditTime: 2023-05-31 21:22:14
...
void upDate(int i)
{
  ...
  switch (i)
  {
    case 0:
    //添加安天祥的信息;
    sprintf(query,"INSERT INTO `drom`.`t_record` SET `studentNumber` = `001` , `studentName` = `安天祥` , `dormBuildId` = `5` ,`dormName` = `233` , `date` = `2023-05-31` WHERE `recordId` = '1';"); 
      break;
    case 1:
    //添加余威望的信息;
    sprintf(query, "INSERT INTO `drom`.`t_record` SET `studentNumber` = `002` , `studentName` = `余威望` , `dormBuildId` = `5` ,`dormName` = `233` , `date` = `2023-05-31` WHERE `recordId` = '2';"); 
      break;
    case 2:
        //添加王正正的信息;
    sprintf(query, "INSERT INTO `drom`.`t_record` SET `studentNumber` = `003` , `studentName` = `王正正` , `dormBuildId` = `5` ,`dormName` = `233` , `date` = `2023-05-31` WHERE `recordId` = '3';"); 
    break;
  }
    if (conn.connected()) { 
    MySQL_Cursor *cur_mem = new MySQL_Cursor(&conn); 
