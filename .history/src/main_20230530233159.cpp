 ...
 * @LastEditTime: 2023-05-30 23:31:46
...
void update(){
  sprintf(str,"%d%d%d",Rxbyte[0],Rxbyte[1],Rx);
  sprintf(query, "insert into t_record values('null','%s','安天祥','4','233','%s','%s')",Rxbyte.c_str(),'2023-06-05','外出'); 
  if (conn.connected()) { 
    MySQL_Cursor *cur_mem = new MySQL_Cursor(&conn); 
    cur_mem->execute(query); 
    delete cur_mem; 
