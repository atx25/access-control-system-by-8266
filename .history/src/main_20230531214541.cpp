 ...
 * @LastEditTime: 2023-05-31 21:45:41
...
void upDate(int i)
{
  ...
  switch (i)
  {
    case 0:
    //添加安天祥的信息;
    sprintf(query,"insert into `drom`.`t_record` (`studentNumber`,`studentName`,`dormBuildId`,`dormName`,`date`,`detail`) values ('001','安天祥','9','233','2023-05-31','neght');"); 
      break;
    case 1:
    //添加余威望的信息;
    sprintf(query, "insert into `drom`.`t_record` (`studentNumber`,`studentName`,`dormBuildId`,`dormName`,`date`,`detail`) values ('002','余威望','9','233','2023-05-31','晚归');"); 
      break;
    case 2:
        //添加王正正的信息;
    sprintf(query, "insert into `drom`.`t_record` (`studentNumber`,`studentName`,`dormBuildId`,`dormName`,`date`,`detail`) values ('003','王正正','9','233','2023-05-31','晚归');"); 
    break;
  }
    if (conn.connected()) { 
    MySQL_Cursor *cur_mem = new MySQL_Cursor(&conn); 
